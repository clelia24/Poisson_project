...
I_exp    <- function(t, beta, theta) (1/50*theta) * exp((beta - 1) * t / 50*theta)
...
simulExp <- function(beta, theta, Tmax) {
  f     <- function(t) I_exp(t, beta, theta)
  M_val <- f(Tmax)
  return(simulPPi(f, Tmax, M_val))
}
...
par(mfrow = c(1, 2))
simE <- simulExp(Beta_val, Theta_val, Tmax_val)
plot.PP(simE); title("Exp Process")
curve(I_exp(x, Beta_val, Theta_val), 0, Tmax_val, col = "red", lwd = 2, main = "Exp Intensity")
...
Power Analysis
We now want to analyze our test’s power. To do so, we will first see how the tests respond to different values of $\beta$ and then to different values of $T_{\max}$.
...
plot_puissance_beta(simulExp, betas_test, Tmax = 100, nsimu = nsimu,
                    theta = 1, titre = "Exp")
```
...
plot_puissance_tmax(simulExp, liste_Tmax, beta_fixe = beta_fixe,
                    nsimu = nsimu, theta = 1, titre = "Exp")
