...
We extract a Poisson process from our dataset by subtracting the dates to get the interarrival times. We then create our Poisson process by the cumulative sum of the interarrival times

```{r}

df$delta_t <- c(NA, as.numeric(diff(df$time), units = "days"))
df[1,3] <- 0
df["new_times"]<-cumsum(df$delta_t)
